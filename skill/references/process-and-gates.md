...
6. Publish up to five calls and five puts. Provide complete analysis for the top three opportunities.
7. Execute only if paper authority is active, every gate passes, and the candidate is `A` or `A_PLUS`. Write a pretrade record first. Real-money execution is never permitted.
...
If paper authority is active and the session is `NO_TRADE`, state the missing condition, the fire trigger, at least one rejected candidate, and whether the block was thesis, timing, liquidity, volatility pricing, catalyst uncertainty, or data quality.
...
Paper fills, if authorized, are a training record. Live remains Owner-only.

Suggested paper instrument set when paper is active:

- long calls 14–45 DTE
- long puts 14–45 DTE
- liquid shares or liquid leveraged ETFs only when an equity expression is superior and documented

Do not treat leveraged ETFs as equivalent to their underlying over multi-day horizons. Path dependence and daily reset must be acknowledged.
